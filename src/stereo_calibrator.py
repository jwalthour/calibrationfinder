...
class StereoCalibrator:
    def __init__(self, calPatternDims=(8, 8), calDotSpacingMm=(25.877, 25.877), detector=None):
        ...
        """
        if detector is None:
            self._calTargetDotDetector = self.make_detector()
        else:
    ...
    def find_single_cam_calibration(self, image_paths):
        ...
        if len(allPointsIn3space) > 0:
            ...
            # for img_path,points,rvec,tvec in zip(image_paths, allPointsIn3space, rvecs, tvecs):
                ...
                # self._drawPointsOnImage(img, imagePoints)
    ...
    def _drawPointsOnImage(self, image, points): 
        """
    ...
    def _findPointVectors(self, image_paths, rowCol=False):
        ...
        for image_path in image_paths:
            ...
            if found:
                ...
                # self._drawPointsOnImage(img, points)
    ...
    def find_stereo_pair_calibration(self, pair_image_paths):
        ...
        allPointsIn3space, all_points_in_left_images = self._findPointVectors(left_image_paths, True)
        logger.info("Finding points in right images from pairs")
        allPointsIn3space, all_points_in_right_images = self._findPointVectors(right_image_paths, True)
        allPointsIn3space = np.array(allPointsIn3space, dtype=np.float32)
        # logger.debug("allPointsIn3space: " + repr(allPointsIn3space))
        ...
        logger.debug("len(allPointsIn3space): " + str(len(allPointsIn3space)))
        ...
        try:
            minError, lCameraMatrix, lDistCoeffs, rCameraMatrix, rDistCoeffs, R, T, E, F = cv2.stereoCalibrate(allPointsIn3space, all_points_in_left_images, all_points_in_right_images, lCameraMatrix, lDistCoeffs, rCameraMatrix, rDistCoeffs, self._IMAGE_SIZE, criteria=stereocalib_criteria, flags=flags)
        except:
    ...
    def find_cal_pattern_in_3space(self, stereo_cal, pair_image_path):
        ...
        # Find individual dots in all the images
        logger.info("Finding points in left images from pairs")
        allPointsIn3space, all_points_in_left_images = self._findPointVectors([pair_image_path[0]])
        logger.info("Finding points in right images from pairs")
        allPointsIn3space, all_points_in_right_images = self._findPointVectors([pair_image_path[1]])
